...
지도학습을 통해 예측한 위험성이 높은 이벤트를 정확하게 탐지하여 기술편차와 침해사고 대응 시간을 단축할 수 있다.
비지도학습은 방화벽, WEB서버의 접속 이벤트에 알고리즘을 적용하여 이상행위를 예측/탐지할 수 있다.

통계적 이상 탐지 기법을 활용하여 악성코드와 같은 비정상 행위도 탐지가 가능하다.
...
QoS (Quality of Service) 통신량을 조절하여 서비스 품질을 유지한다.

application(사용자) ->  Link(통신망외부로 나가는)
사용자는 link를 통해서 데이터를 보내고 받는다.
그 중간에 gate가 얼마 만큼의 패킷을 보내려는 건지랑 링크 사용량을 측정한다.

보내는 패킷량의 기준을 100으로 하고 100보다 많으면 막아야 한다. 버퍼에 쌓아서 순차적으로 보낸다.
100보다 작은 50이라면 버퍼의 50과 합쳐서 보내는 등의 규칙

통신량의 사용량이 많으면 게이트에서 사용자를 위해서 일정량만 보내도록 속도를 줄인다.

역으로 통신량의 사용량이 적으면 버퍼에 쌓아논 것을 보내서 더 많이 보내도록 하는 규칙을 사용하겠다.

얼만큼이 high, low, 얼만큼이 reduce, increase인지에 대해서 적절한지 의문을 갖는다.
퍼지 논리에서는 많고 적음의 양을 membership functions 멤버쉽 값이는 것으로 정의 하겠다.

기준을 정해놓고 값을 넣으면 참, 거짓이 나온다.
x가 170보다 작으면 참, 거짓, x가 170이상 180이하라면 또 참 거짓

반면에 퍼지 이론에서는 멤버쉽함수를 도입을 한다.
x가 170보다 작으면 작다=1 기준점이 170

173을 넣으면 0.3이나온다. 작다에 속하는 정도가 0.3정도가 나온다.

170보다 180 보통이다.

180이상이면 크다.

f(x) 작다라는 함수에 173이라는 숫자를 넣으면 0.3이나온다. 작음의 정도가 0.3
f(x) 중간인가에 173이라는 숫자를 넣으면 1이나온다. 작음의 정도가 1

퍼지 이론에 의하면 값을 각각 다른 함수에 적용해서 다른 결과가 나온다.
애매모한 기준에 대해서 결과를 얻을 수 있도록 설계했다

만일 if(패킷의 양이 많으면) 송신량을 줄여라. QoS 서비스 품질을 위해서
만일 if(패킷의 양이 적으면) 송신량을 높여라. 

얼만큼이 많고 적은지를 정의해야 하는 문제상황

100보다 크면 많은거고 100보다 작으면 작은것이다. 기준점을 정해서(100)으로 정해서

왼쪽에서 작다의 정도를 멤버쉽함수로, 크다의 정로를 오른쪽에서 멤버쉽함수로 정의

90은 80보다는 덜 작은거고 작다라고 할 수 있는 정도가 0.3``

```
![image](https://github.com/mr-won/AI_Robot/assets/58906858/f6f76498-58b4-459b-8dac-5331f8ac72f6)   
![image](https://github.com/mr-won/AI_Robot/assets/58906858/b512b689-754c-4b5f-b538-1586e7806eb0)
```
100을 기준으로
10일 때 ROW에 속하는 정도가 1.0 HIGH는 0.0 10은 아주 낮음 -> 키워야함
85일 때 row에 속하는 정도가 0.75
90일 때 row에 속하는 정도가  0.5 
95일 때  row에 속하는 정도가 0.25
```
![image](https://github.com/mr-won/AI_Robot/assets/58906858/3953dec5-7283-47cd-a3f6-d97394524344)
```
pdelta 얼마나 늘일것,줄일것인가를 결정하는 초매개변수 책에서는 10으로 했다. 튜닝

rate는 게이트가 조절해야 할 양 QoS를 유지하기 위해서 조절해야 할 양

현재 rate만큼 보내고 있는데 100을 기준으로 얼마나 많고 적은지를 보고 다음 단계의 rate를 수정해라

rate = rate(현재) + (low(rate) * pdelta) - (high(rate) * pdelta) (현재의 양을 보고 다음 rate에 값을 수정)

packet rate 90일때 다음 paket의 양은

90 = 90 + (0.5 * 10) - (0.0 * 10) = 다음단계

90 = 90 + 5 - 0 = 95 다음 단계는 95가 됨

패킷의 양에 맞게 값의 수정치를 수정한다. 실제 세계에서와 같이 값을 수정할 수 있다.

80이면 늘려야한다.
80 = (1*10)-(0*10) = 90 많이 키운다.

100을 유지함으로써 서비스 품질을 자동적으로 유지할 수 있게 된다.

```
![image](https://github.com/mr-won/AI_Robot/assets/58906858/39027213-c3cb-4e6f-89d3-f6d67528c781)
```
직선의 방정식 

x가 100보다 크면 0, 80보다 작으면 1, 80과 100 사이면 직선의 방정식

low(x) x가 80, 100 사이라면
(80,1), (100, 0) 두 점을 지나는 직선의 방정식을 구하면 (100-rate)/20


high(x) x가 100, 120 사이라면
(100,0), (120, 1) 두 점을 지나는 직선의 방정식 (rate-100)/20
```

## 23.11.29
```
0도일때는 아예 정면인데 0.2정도되면 0.8정도 가운데 extreme left, far left
far right, extreme right 멤버쉽을 구분

나랑 먹이와의 각도가 0도가 되는 방향으로 쫒아가야 먹이를 먹을 수 있다.
내가 쫒아가는 각도가 1도라도 틀어져있다면 먹이를 먹을 수 없다.
error angle <- 틀어져있는 각도

center 0도로부터 왼쪽, 오른쪽의 양에 따라 내가 회전해야 하는 정도를 정한다.

나는 100, 100에서 시작하고 먹이는 랜덤한 위치에서 만들어진다. (84, 30)

나하고 먹이의 방향이 꽤 오른쪽이기때문에 회전해야 한다.
...
and -> min
```
![image](https://github.com/mr-won/AI_Robot/assets/58906858/682391d5-6a63-4634-a96c-c273168d4c4c)    
![image](https://github.com/mr-won/AI_Robot/assets/58906858/ed7009af-c237-49fe-a2c3-25fac23e9cd0)    
```
고속충전기 -> 배터리 full -> 오류가 높아짐

온도가 너무 높아지면 저속충전으로 바꾼다.
온도가 낮거나 전압이 높으면

온도랑 볼트를 사용하는데 봍트 

볼트가 중간정도다. 볼트가 5~10이면 낮다고 할수도 있고 높다고 할수도 있다.
...
별도의 퍼지집합을 정의했는데
그냥 left, right 가지고 very나 not very를 사용해서 정도의 집합을 늘릴 수 있다.

hedge 씌우다. 퍼지집합에 very나 not very를 취해서 멤버쉽 값을 정할 수 있다.

패킷의 속도가 115일 때 높다고 할 수 있는 정도가 0.75였다면 115일 때 높다라고 할 수 있다. 아주 높니 very라고 물어보면 아주 높진 않다. 실제로 루트를 취하면 수치가 0.75에서 0.5로 내려가서 확실하게 아주 높진 않다고 얘기를 할 수 있게 된다.

trickle 저속 fast 고속

배터리가 충전이 다 되었는데 전류가 흐르게 되면 열이 발생한다.

뜨거워지면 배터리가 완전히 충전되었다는 것을 의미하며 고속충전모드를 저속모드로 바꾼다.
일정한 전압에 도달하게 되면 충분한 전압으로 고속충전은 필요없다.

세 개의 규칙을 사용한다.
전압이 높으면 모드를 저속모드로 바꿔라.
온도가 높으면 저속모드로 바꿔라
볼트가 높지않고 온도가 높지 않으면 고속모드로 해라

not이라는 hedge가 나온다. and 집합연산도 나온다.

volt는 low medium high 멤버쉽함수를 정함

7 정도되면 low medium에 겹침

온도는 cold, warm, hot 멤버쉽함수를 정함

15~45도는 어느정도는 차갑다고 할수있고 따뜻하다고 할 수 있다.

볼트, 온도를 둘다 측정한다. 전압이 높냐라는 멤버쉽 값에 0.75 정도 높다고 하고
온도가 높냐고 물었더니 멤버쉽 값이 0.5 정도 and 인경우 계산값이 0.8

or면 0.75 큰 값을 택한다. and 일 때는 작은 값을 택한다.
1-0.75 = 0.25
1-0.8 = 0.2
or면 큰값, and면 작은값
...
따로따로 계산한다.
```
![image](https://github.com/mr-won/AI_Robot/assets/58906858/04cf35ba-a3df-4fda-b79a-5f8674b5ca5c)
```
수행한 결과 그래프는 빨간생 volt와 온도 temp가 낮을 때 고속 충전을 한다.

고속충전을 하는동안 온도는 높아진다. 전압은 내려갓다.
...
## 12.05 Bigram Model
```
Markov model을 변형한 것이 bigram model이다.

상태와 상태들 사이의 변화가 있는데 변화할 확률과 변화를 짝으로 지어진 모델이다.

text 자동 생성 마이코프 모델

마디코프 체인은 상태와 그 상태들 간의 전이확률로 구성되어 있는 시스템이다'''
변이가 연쇄적으로 일어나기 때문에 체인이라고 한다.
표시가 없는 건 1이다.

상태는 1.이전 상태에 의해 주어진 확률에 따라 결정된다. Marchov model
이것이 마르코프다. 2. 이전상태를 기억하지 않는(이전 상태가 많아서) 히든 마르코프 모델 hmm이라고한다. 

응용

바이그램 모델을 작성한다.
현재 상태는 일련의 (하나의) 이전 상태로부터 현재 상태에 도달햇다.
직전 상태들의 영향을 받지 않는다.
...
n = 2 bigram n=3 trigram n-ngram

더 많은 이전 상태를 가지고 확률을 예측하는 것이 일밙거으로는 좋다.
