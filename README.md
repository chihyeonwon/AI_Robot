...
지금 여기서 시스템이 microsoft age of kings에 사용되었다. 게임인듯

타이머->턴방식
```
## 23.11.28 퍼지 이론 Fuzzy Logic
```
A = { }
5보다 큰 수의 집합이라 할 때 정수 3은 포함되지 않는다. A = {6, 8} 은 집합이다.
하지만 어떤 집합 B가 애매모호한 요소들로 이루어졌을 때 집합이 아니다.

크다라는 개념이 모호하다. 어떤 대상인가에 대해서, 얼마나 큰지도 모호하다.

실세계문제랑 컴퓨터세계의 차이(disparity)을 극복(overcome)하기 위해서 퍼지 시스템 이론을 정의했다.

컴퓨터의 세계는 참 거짓 두 개의 논리 시스템이다. two valued

컴퓨터과학의 핵심적인 부분은 이산수학, 자료구조, 알고리즘이다.

퍼지논리에서는 얼마나 그런가 degrees of thruth으로 설명한다.

0하고 1 사이 (0과 1을 포함한 사이) between 0.0 and 1.0 inclusive
